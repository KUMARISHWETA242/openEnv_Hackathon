...
class SatelliteEnvironment(Environment):
    ...
    Example:
        ...
        >>> obs = env.reset()
        >>> print(obs.echoed_message)  # "Satellite environment ready!"
        >>>
        >>> obs = env.step(SatelliteAction(message="Hello"))
    ...
    def reset(self) -> SatelliteObservation:
        ...
        Returns:
            SatelliteObservation with a ready message
        """
        self._state = State(episode_id=str(uuid4()), step_count=0)
        self._reset_count += 1

        return SatelliteObservation(
            echoed_message="Satellite environment ready!",
            message_length=0,
            done=False,
            reward=0.0,
        )
    ...
    def step(self, action: SatelliteAction) -> SatelliteObservation:  # type: ignore[override]
        ...
        Args:
            action: SatelliteAction containing the message to echo

        Returns:
            SatelliteObservation with the echoed message and its length
        """
